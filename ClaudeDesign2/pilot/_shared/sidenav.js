 ...
 *   • Slim-mode toggle (70px icons-only) — Nimbus-native feature
 *   • 3px left accent bar on active items via box-shadow inset (Nimbus pattern)
 *   • Sub-items at Nimbus-spec 36px height / 0.78rem font-size
 *   • Logo max-height 40px (was 48px)
 *   • Uses CSS var(--cnds-product-accent) — automatically picks up Expireon purple
 *     when <body class="cnds-product-expireon"> is set
 *   • Header background uses --cnds-sidenav-bg token, not body bg
...
<nav id="sidebar-cf" class="sidenav show flex-shrink-0"
  ...
  <div class="sidenav-header">
    <img src="_shared/expireon-logo.png" alt="Expireon">
  ...
  <ul class="sidenav-menu flex-1">
    ...
    ${collapseStart(legalOpen)}
      ${sub('legal-cases.html', 'legal_cases', 'Legal Cases')}
    ${collapseEnd()}
  ...
  // ── Style overrides (Nimbus-spec'd) ──────────────────────────────────────
  // Nimbus default .sidenav is position:fixed and shadow-3, transformed off-screen.
  // For a static admin shell we override to position:relative with no shadow.
  // We also implement the Nimbus product-accent 3px active bar.
  const styleId = '_shared-sidenav-styles';
  if (!document.getElementById(styleId)) {
    const style = document.createElement('style');
    style.id = styleId;
    style.textContent = `
      /* Static, in-flow sidenav (override Nimbus's offcanvas defaults) */
      ...
      /* Header — sized to match topnav height (64px) */
      ...
      /* Active state — Nimbus pattern: 3px left accent bar + product highlight bg */
      ...
      /* Category toggles when expanded: highlight bg but no accent bar */
      .sidenav[data-purpose="shared-sidenav"] .sidenav-link[data-cnds-toggle="sidenav-collapse"].active {
        box-shadow: none;
        font-weight: 500;
      }

      /* Sub-items (L2 + L3) — Nimbus spec: 36px height, 0.78rem font */
      ...
      .sidenav[data-purpose="shared-sidenav"] .sidenav-subsublink.active {
        padding-left: 3rem; /* preserve indent on active */
      }
      ...
      /* Slim mode: hide labels & chevrons, center icons */
